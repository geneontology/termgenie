...
public class CommitHistoryTools {
	...
	public static CommitedOntologyTerm create(Frame frame,
	...
	public static boolean update(CommitedOntologyTerm term,
			Frame frame,
			Set<OWLAxiom> axioms,
			Modification operation)
	{
	...
	public static SimpleCommitedOntologyTerm createSimple(Pair<Frame, Set<OWLAxiom>> pair,
			Modification operation)
	{
	...
	private static List<CommitedOntologyTerm> translateTerms(List<CommitObject<TermCommit>> terms) {
		...
		if (terms != null && !terms.isEmpty()) {
			...
			for (CommitObject<TermCommit> commitObject : terms) {
				...
				CommitedOntologyTerm term = create(frame,
						commitObject.getType(),
						commitObject.getObject().getPattern());
