...
class BasicRules extends BasicTools {
	...
	protected List<Synonym> synonyms(String prefix,
			OWLObject x,
	...
	protected String createDefinition(String prefix,
			List<OWLObject> list,
			OWLGraphWrapper ontology,
			String infix,
			String suffix,
	...
	protected static class CDef {
		...
		protected void addDifferentium(String rel,
	...
	protected List<TermGenerationOutput> createTermList(String label,
			String definition,
	...
	protected void createTermList(String label,
			String definition,
			List<Synonym> synonyms,
			CDef logicalDefinition,
			TermGenerationInput input,
			...
			List<TermGenerationOutput> output)
	{
		...
		if (sameName != null) {
			output.add(singleError("The term " + ontology.getIdentifier(sameName) + " with the same label already exists",
					input));
			return;
		}
