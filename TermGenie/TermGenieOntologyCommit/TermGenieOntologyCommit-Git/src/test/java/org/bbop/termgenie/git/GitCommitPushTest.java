...
public class GitCommitPushTest {
	...
	public void beforeClass() throws Exception {
		...
		// general layout
		repository = new File(currentFolder, "repository");
		FileUtils.forceMkdir(repository);
		
		readOnly = new File(currentFolder, "readonly");
		FileUtils.forceMkdir(readOnly);
		
		TestResourceLoader loader = new TestResourceLoader();
		
		Git git = Git.init().setDirectory(repository).call();
		
		// file staging with content
		File ontologyFolder = new File(repository, "ontology");
		...
		git.add().addFilepattern("ontology/git-test-main.obo").call();
		git.commit().setAuthor("test-user", "foo@foo.foo").setMessage("initial import").call();
	...
	public void simpleCommit() throws Exception {
		...
		OboScmHelper helper = new TestGitHelper("ontology/git-test-main.obo", repository.getAbsolutePath());
		...
		GitTool gitTool = new GitTool(repository.getAbsolutePath(), new File(currentFolder, "verification"), null, null);
		gitTool.connect();
		boolean checkout = gitTool.checkout(Arrays.asList("ontology/git-test-main.obo"), ProcessState.NO);
		gitTool.close();
		assertTrue(checkout);
		
		OBOFormatParser parser = new OBOFormatParser();
		File local = gitTool.getTargetFolder();
		OBODoc mainObo = parser.parse(new File(local, "ontology/git-test-main.obo"));
	...
	private OntologyTaskManager loadOntology(Ontology ontology) throws Exception {
		final GitTool gitTool = new GitTool(repository.getAbsolutePath(), readOnly, null, null);
		try {
			gitTool.connect();
			boolean checkout = gitTool.checkout(Arrays.asList("ontology/git-test-main.obo"), ProcessState.NO);
			gitTool.close();
		...
		OntologyTaskManager source = new OntologyTaskManager(ontology) {
			...
			protected OWLGraphWrapper createManaged() {
				try {
					...
					gitTool.connect();
					boolean update = gitTool.update(Arrays.asList("ontology/git-test-main.obo"), ProcessState.NO);
					assertTrue(update);
					gitTool.close();
					File local = gitTool.getTargetFolder();
					OBODoc mainObo = p.parse(new File(local, "ontology/git-test-main.obo"));
	...
	private static final class TestGitHelper extends OboScmHelper {

		private final String repository;

		private TestGitHelper(String mainOntology, String repository)
		{
		...
		{
			return new GitTool(repository, scmFolder, null, null);
		}
	}
