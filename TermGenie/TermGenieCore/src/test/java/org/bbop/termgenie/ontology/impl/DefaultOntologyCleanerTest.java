...
public class DefaultOntologyCleanerTest {
	...
	public void testLoadCleanerConfig() throws IOException {
		...
		assertSetEquals(new String[] { "ts1", "ts2" },
				cleanerConfig1.retain_term_clauses.get("ts").types);
		...
		assertSetEquals(new String[] { "td1" },
				cleanerConfig1.retain_typedef_clauses.get("td").types);
		...
		assertSetEquals(new String[] { "ts1", "ts2" },
				cleanerConfig2.retain_term_clauses.get("ts").types);
		...
		assertSetEquals(new String[] { "td1" },
				cleanerConfig2.retain_typedef_clauses.get("td").types);
	...
	public void testGuice() {
		...
		assertTrue("Assert the singleton pattern",
				cleaner == injector.getInstance(OntologyCleaner.class));
	}
