...
public class DefaultTermTemplates {
	...
	DefaultTermTemplates(OntologyConfiguration configuration) {
		...
		TemplateField Field_Target_Regulation_BP = new TemplateField("target", true, TemplateField.SINGLE_FIELD_CARDINALITY, Arrays.asList("regulation","negative_regulation","positive_regulation"), GENE_ONTOLOGY_BP);
		TemplateField Field_Target_Regulation_MF = new TemplateField("target", true, TemplateField.SINGLE_FIELD_CARDINALITY, Arrays.asList("regulation","negative_regulation","positive_regulation"), GENE_ONTOLOGY_MF);
		...
		all_regulation = create(GENE_ONTOLOGY, "all_regulation", "regulation: biological_process",
		...
		all_regulation_mf = create(GENE_ONTOLOGY, "all_regulation_mf", "regulation: molecular_function",
		...
		part_of_cell_component = create(GENE_ONTOLOGY, "part_of_cell_component",
		...
		abnormal_morphology = create(HP_ONTOLOGY, "abnormal_morphology",
				"Abnormal X morphology",
				createRules("Dummy Rule","Dummy Name Rule\n2nd line"),
				requiredPlusOptionalFields(new TemplateField("target", FMA_ONTOLOGY)));
		
		hpo_entity_quality = create(HP_ONTOLOGY, "hpo_entity_quality",
				"basic EQ template",
				createRules("Dummy Rule","Dummy Name Rule\n2nd line"),
				requiredPlusOptionalFields(
		...
		omp_entity_quality = create(OMP, "omp_entity_quality",
				"basic EQ template",
				createRules("Dummy Rule","Dummy Name Rule\n2nd line"),
				requiredPlusOptionalFields(
		...
		metazoan_location_specific_cell = create(CELL_ONTOLOGY, "metazoan_location_specific_cell",
		...
		cell_by_surface_marker = create(CELL_ONTOLOGY, "cell_by_surface_marker",
		...
		metazoan_location_specific_anatomical_structure = create(UBERON_ONTOLOGY, "metazoan_location_specific_anatomical_structure",
	...
	private TermTemplate create(Ontology correspondingOntology, String name, String description, String rule,
			TemplateField...fields) {
		return create(correspondingOntology, name, description, createRules(rule, null), fields);
	}
	
	private TermTemplate create(Ontology correspondingOntology, String name, String description, List<TemplateRule> rules,
			TemplateField...fields) {
		return create(correspondingOntology, name, description, null, rules, fields);
	}
	
	private TermTemplate create(Ontology correspondingOntology, String name, String description, String hint, List<TemplateRule> rules,
			TemplateField...fields) {
		return create(correspondingOntology, name, null, description, hint, rules, fields);
	}
	
	private TermTemplate create(Ontology correspondingOntology, String name, String displayName, String description, String hint, List<TemplateRule> rules,
			TemplateField...fields) {
	...
	private List<TemplateRule> createRules(String termRule, String nameRule) {
...
//		TermTemplateIO writer = new FlatFileTermTemplateIO();
