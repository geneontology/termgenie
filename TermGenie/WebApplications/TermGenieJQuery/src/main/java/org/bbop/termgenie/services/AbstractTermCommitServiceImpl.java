...
public abstract class AbstractTermCommitServiceImpl extends NoCommitTermCommitServiceImpl {
			...
			String ontologyName)
	{
		...
		if (!getTargetOntology().getUniqueName().equals(manager.getOntology().getUniqueName())) {
			return error("Can only commit to GeneOntology, but requested ontology was: " + ontologyName);
		}
		...
		// check if user has permissions for trying a commit
			...
			OntologyIdProvider idProvider) throws CommitException
	{
		...
		Set<String> missingIds = new HashSet<String>(idHandler.used);
		missingIds.removeAll(idHandler.tempIdMap.keySet());
	...
	private class CommitTask implements ManagedTask<OntologyIdProvider> {
		...
		@Override
		public ManagedTask.Modified run(OntologyIdProvider idProvider) {
			...
			try {
				...
				return ManagedTask.Modified.no;
			}
