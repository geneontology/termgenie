...
public class GenerateTermsServiceImpl extends RemoteServiceServlet implements GenerateTermsService {
	...
	public GenerationResponse generateTerms(String ontology,
		...
		if (ontology == null || ontology.isEmpty()) {
			return new GenerationResponse(NO_ONTOLOGY, null, null);
		}
		if (allParameters == null) {
			return new GenerationResponse(NO_TERM_GENERATION_PARAMETERS, null, null);
		}
		if (commit) {
			if (username == null || username.isEmpty()) {
				return new GenerationResponse(MISSING_USERNAME, null, null);
			}
		}
		...
		for (Pair<GWTTermTemplate, GWTTermGenerationParameter> pair : allParameters) {
			if (pair == null) {
				return new GenerationResponse("Unexpected null value.", null, null);
			}
			...
			if (template == null) {
				return new GenerationResponse("Unknow template specified: " + one.getName(), null, null);
			}
			GWTTermTemplate gwtTermTemplate = GWTTemplateTools.createGWTTermTemplate(template);
			
			List<GWTValidationHint> errors = FieldValidator.validateParameters(gwtTermTemplate,
					parameter);
			if (!errors.isEmpty()) {
		...
		if (!allErrors.isEmpty()) {
			return new GenerationResponse(null, allErrors, null);
		}
		if (commit) {
			...
			if (!valid) {
				return new GenerationResponse(UNKOWN_USERNAME_PASSWORD, null, null);
			}
		}
		// TODO add call to reasoning engine
		// return response
		return new GenerationResponse(null, null, Collections.singletonList("Dummy Response."));
	}
